...
let COMMANDS_ROOT = './commands';
let COMMAND_EXTENSIONS = ['.js', '.ts', '.jsx', '.tsx'];
...
function rewatch() {
  ...
  let wpWatcher = wpCompiler.watch({}, (err, stats) => onWebpackFinish(err, stats));
  ...
  function onWebpackFinish(err, stats) {
    ...
    printBuildStats(fpConfig, stats);
  }
}
...
async function build() {
  let config = prepareFigpackConfig();
  let wpCompiler = makeWebpackCompiler(config);
  ...
  wpCompiler.run((err, stats) => {
    ...
    writeExtraFiles(config);
    printBuildStats(config, stats);
  });
}
...
/**
 * Prints out build info upon rebuild
 */
function printBuildStats({ commands }, wpStats) {
  // console.log(wpStats.toString({ modules: false, colors: true }));
  let cmds = commands.length === 1
    ? `1 command`
    : `${commands.length} commands`;
  console.log(`Built plugin (${cmds}) @ ${(new Date()).toISOString()}`);
}
